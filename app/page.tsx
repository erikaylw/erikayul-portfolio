...
function useCounter(target: number, duration: number = 600) {
    ...
    >
      <div className="flex items-center gap-6">
        ...
        <div className="hidden items-center gap-5 text-slate sm:flex">
          ...
          <a href="#expertise" className="transition-colors hover:text-accent">
            Expertise
          </a>
          ...
          <a
            href="#contact"
            className="rounded-full bg-navy px-4 py-1.5 text-white transition-colors hover:bg-accent"
          >
            Contact
          </a>
        </div>
...
function StatBlock({ value, suffix, label }: { value: number; suffix: string; label: string }) {
  ...
  return (
    <div className="stat-figure">
      <span ref={ref} className="font-display text-5xl font-bold tracking-tight text-accent sm:text-6xl">
        {count}
        {suffix}
      </span>
      <p className="mt-1 text-sm font-semibold text-slate">{label}</p>
    </div>
  );
}
    ...
    <>
      ...
      <main className="min-h-screen bg-mist text-navy">
        ...
        <section className="overflow-hidden pt-24 sm:pt-28">
          <div className="mx-auto grid max-w-6xl items-center gap-10 px-5 pb-16 sm:px-8 md:pb-24 lg:grid-cols-[1.1fr_0.9fr]">
            <div>
              <div className="stat-figure font-display text-7xl font-bold leading-none tracking-tight text-accent sm:text-8xl lg:text-9xl">
                5+
              </div>
        ...
        <section className="border-y border-line bg-white">
          <div className="mx-auto grid max-w-6xl grid-cols-2 gap-8 px-5 py-10 sm:px-8 md:grid-cols-4">
            <StatBlock value={5} suffix="+" label="Years Experience" />
            <StatBlock value={4} suffix="" label="Companies Served" />
            <StatBlock value={3} suffix="" label="Professional Certifications" />
            <StatBlock value={1} suffix="" label="Focus: Tax & Accounting" />
          </div>
        </section>
          ...
          <div className="mx-auto max-w-3xl px-5 py-20 sm:px-8 sm:py-28">
            <div className="reveal">
              ...
              <div className="mt-6 space-y-5 text-base leading-7 text-slate sm:text-lg sm:leading-8">
                <p>
                  Profesional Akuntansi dan Perpajakan dengan pengalaman lebih dari 5 tahun dalam tax compliance,
                  pelaporan keuangan, rekonsiliasi bank, dan administrasi perpajakan perusahaan. Berpengalaman
                  menangani PPN, PPh 21, PPh 23, PPh 4 Ayat (2), SPT Masa dan Tahunan, serta menggunakan Accurate,
                  Harmoni, Coretax, dan Microsoft Excel.
                </p>
                <p>
                  Berbasis di Surabaya, Indonesia. Terbuka untuk peluang Tax Staff, Tax & Accounting Staff, Finance
                  Administration, dan Accounting Staff di lingkungan corporate yang profesional.
                </p>
              </div>
              <div className="mt-8 flex flex-wrap gap-2">
                {targetRoles.map((role) => (
                  <span
                    key={role}
                    className="rounded-full border border-line bg-white px-4 py-1.5 text-sm font-semibold text-slate"
                  >
                    {role}
                  </span>
                ))}
          ...
          <div className="mx-auto max-w-5xl px-5 py-20 sm:px-8 sm:py-28">
            ...
            <div className="mt-12 grid gap-6 md:grid-cols-2">
              {expertiseGroups.map((group, i) => (
                <div key={group.title} className="reveal border-t border-line pt-5" style={{ animationDelay: `${i * 100}ms` }}>
                  <h3 className="font-display text-xl font-bold text-navy">{group.title}</h3>
                  <div className="mt-4 flex flex-wrap gap-2">
                    {group.items.map((item) => (
                      <span
                        key={item}
                        className="rounded-full border border-line bg-mist px-3.5 py-1.5 text-sm font-medium text-slate"
                      >
                        {item}
                      </span>
                    ))}
          ...
          <div className="mx-auto max-w-5xl px-5 py-20 sm:px-8 sm:py-28">
            ...
            <div className="mt-12 space-y-10">
              {experiences.map((exp, i) => (
                <div key={`${exp.company}-${exp.period}`} className="reveal" style={{ animationDelay: `${i * 120}ms` }}>
                  <div className="grid gap-2 sm:grid-cols-[7rem_1fr] sm:gap-6">
                    <p className="pt-0.5 text-sm font-semibold uppercase tracking-[0.12em] text-accent">
                      {exp.period}
                    </p>
                    <div>
            ...
            <div className="grid gap-14 lg:grid-cols-[1fr_1.1fr]">
              ...
              <div>
                ...
                <div className="mt-10 grid grid-cols-2 gap-4">
                  {certifications.map((cert, i) => (
                    <div
                      key={cert.title}
                      className="reveal overflow-hidden rounded-xl border border-line bg-mist"
                    ...
                    >
                      <div className="relative aspect-[4/3] border-b border-line bg-white">
                        {cert.image ? (
                          ...
                          <div className="flex h-full items-center justify-center px-4 text-center text-sm font-medium text-slate">
              ...
              <div className="flex flex-col gap-10">
                ...
                <div className="reveal">
                  ...
                  <h2 className="mt-4 font-display text-3xl font-bold leading-tight tracking-tight text-navy">
                    Academic foundation.
                  </h2>
                  <div className="mt-6 rounded-xl border border-line bg-mist p-6">
                    ...
                    <ul className="mt-5 space-y-2">
                      <li className="relative pl-5 text-sm leading-6 text-slate before:absolute before:left-0 before:top-2.5 before:h-1 before:w-1 before:rounded-full before:bg-accent">
                        Finalis National Capital Market Championship 2019
                      </li>
                      <li className="relative pl-5 text-sm leading-6 text-slate before:absolute before:left-0 before:top-2.5 before:h-1 before:w-1 before:rounded-full before:bg-accent">
                        Relawan Pajak / Pojok Pajak
                      </li>
                    </ul>
                  </div>
                </div>
                ...
                <div className="reveal">
                  ...
                  <h2 className="mt-4 font-display text-2xl font-bold leading-tight tracking-tight text-navy">
                    Digital Finance &amp; Web3
                  </h2>
                  <p className="mt-3 text-base leading-7 text-slate">
                    Memiliki exposure tambahan dalam digital finance dan Web3 ecosystem melalui pengalaman business
                    development dan market reporting pada industri blockchain.
                  </p>
          ...
          <div className="mx-auto max-w-5xl px-5 sm:px-8">
            <div className="mx-auto max-w-2xl text-center">
              ...
              <p className="mt-4 text-base leading-7 text-white/60">
                Terbuka untuk peluang Tax Staff, Tax Administration, Tax &amp; Accounting Staff, Finance
                Administration, dan Accounting Staff.
              </p>
            </div>
            <div className="mx-auto mt-10 grid max-w-lg gap-3 sm:grid-cols-2">
              <a
                href={contactLinks.email}
                className="inline-flex items-center justify-center gap-2 rounded-full bg-white px-6 py-3.5 text-sm font-bold text-navy transition-colors hover:bg-accent hover:text-white"
              >
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="inline-flex items-center justify-center gap-2 rounded-full border border-white/20 px-6 py-3.5 text-sm font-bold transition-colors hover:border-accent hover:text-accent"
              >
              ...
              <a
                href={contactLinks.cv}
                className="inline-flex items-center justify-center gap-2 rounded-full bg-accent px-6 py-3.5 text-sm font-bold text-white transition-colors hover:bg-accent/80"
              >
      ...
      <footer className="border-t border-line bg-white px-5 py-6 text-center">
        <p className="text-sm font-medium text-slate">
          Erika Yuliani Lukmawati — Tax &amp; Accounting Professional
        </p>
      </footer>
    </>
  );
